    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if epoch % args.save_freq == 0:
            torch.save(model.state_dict(),
                       os.path.join(model_dir, 'TRADES-'+args.dataset+'-model-'+args.model+'-epoch{}.pt'.format(epoch)))
            torch.save(optimizer.state_dict(),
                       os.path.join(model_dir, 'opt-TRADES-'+args.dataset+'-'+args.model+'-checkpoint_epoch{}.tar'.format(epoch)))
