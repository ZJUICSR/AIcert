...
def error_level(pred_test_prob,true_test):#新增评价指标：严重性指标     
    ...
    return error_level
def nac_test(model, device,model_name,dataset_name,data,true_test):
    model.eval()
    ...
    if dataset_name=='cifar':
        df.to_csv('output/cache/test_case_select/all_output/output_cifar/{}/{}_nac_0.csv'.format(model_name,dataset_name))
    if dataset_name=='fashionminist':
        df.to_csv('output/cache/test_case_select/all_output/output_fashionminist/{}/{}_nac_0.csv'.format(model_name,dataset_name))
...
if __name__=='__main__':
    ...
    if args.white_box_attack:
        ...
        nac_test(model, device,model_name,dataset_name,data,true_test)
