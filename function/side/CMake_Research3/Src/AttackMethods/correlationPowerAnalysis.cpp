...
//---------------优化--------------------------------------------------------
void generate_midvaluehw_samples(Parameters* param, int8_t* (*f)(Parameters*), float** sample, uint8_t** hw){
...
void correlationPowerAnalysis_correlation_distinguish_optimize(Parameters* param, int8_t* (*f)(Parameters*)){
    ...
    float* res_cor = (float*)malloc(param->getPointNum() * sizeof(float));
    ...
    //计算mid 读入samples
    generate_midvaluehw_samples(param, f, sample, hw);
    ...
    for(int index = 0; index < (param->getTraceNum()-bias)/gap+1; index++){
        ...
        for (int i = 0; i < param->getGuessSize(); i++){
            for (int j = 0; j < param->getPointNum(); j++){
                ...
                //问题出现在这行，检查内存是否溢出？
                res_cor[j] = (float)abs(BaseTools::corr(hw[i], sample[j], bias + index * gap));
                if(i==128){
                    res_cor[j] = 0;
                }
            }
            trsData_result[i].samples[index]=res_cor[BaseTools::findMaxCorr(res_cor,param->getPointNum())];
        }
        #if 0
        printf("%d_max:%f;\n",i-128,result_max[i]);
        #endif
