    ...
    def _simple_lrp(self, R, labels, device, r_method="composite", beta=0):
        ...
        for key, module in enumerate(reversed(self.layers)):
            ...
            if key == 0:
                ...
                out_p = torch.where(self.out < 0, torch.zeros(1).to(device), self.out)
                out_n = self.out - out_p

                x_p = torch.where(self.x < 0, torch.zeros(1).to(device), self.x)
                x_n = self.x - x_p
...
class ResNet(Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False, whichScore=None):
        ...
        self.fc = Linear(512 * block.expansion, num_classes, whichScore=whichScore, lastLayer=True)
