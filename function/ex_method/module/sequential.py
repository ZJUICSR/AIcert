...
class Sequential(Module):
    ...
    def location(self, LRP, device, keep_batch=False, interpreter='lrp', lrp_target_layer=None):
        l = LRP.shape[2]
        n = LRP.shape[0]
        ...
        if interpreter == 'lrp' and lrp_target_layer == None:
            LRP = LRP / LRP.max()
            mask[:, :, k:-k, k:-k] = -1 * torch.ones((l - 2 * k, l - 2 * k), dtype=torch.float32).to(device)
        else:
            mask[:, :, k:-k, k:-k] = torch.zeros((l - 2 * k, l - 2 * k), dtype=torch.float32).to(device)
    ...
    def corner(self, LRP, device):
        LRP = self.normalize(LRP)
        l = LRP.shape[2]
        mask = torch.zeros_like(LRP)
        k = (l // 7) * 2
        mask[:, :, :k, :k] = torch.ones((k, k), dtype=torch.float32).to(device)
    ...
    def center_mass(self, LRP, LRP_label, device, keep_batch=False):
        n, c, h, w = LRP.shape
        LRP = self.normalize(LRP)
        ...
        h_vector = torch.range(1, h).reshape(1, 1, h, 1).to(device)
        w_vector = torch.range(1, h).reshape(1, 1, 1, w).to(device)
    ...
    def interpretation(self, activation_output, interpreter, labels, num_target, device, target_layer=None, inputs=None,
        ...
        if interpreter == 'lrp':
            ...
            LRP = self.integrated_grad(inputs, labels, None, device, num_target)

        elif interpreter == 'integrated_grad_T':
            LRP = self.integrated_grad(inputs, labels, target_layer, device, num_target)
    ...
    def integrated_grad(self, inputs, labels, target_layer, device, num_target=1000, smooth_num=30):
