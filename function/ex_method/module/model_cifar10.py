...
class Network_cifar10(Module):
    ...
    def __init__(self, parameters):
        ...
        self.feature = Sequential(
            Conv2d(in_channels=3, out_channels=6, kernel_size=parameters["conv_k"], stride=parameters["conv_stride"], padding=parameters["conv_padding"]),
            ReLU(),
...
def cifar_self(pretrained=False, reference_model=None):
    checkpoint = torch.load("models/cifar10_model.pkl")
    parameters = checkpoint["parameters"]
    if pretrained == False:
        net = Network_cifar10(parameters)
        return net.feature
    else:
