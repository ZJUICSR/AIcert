...
def run_modulardevelop(dataset,model,tuner,init, epoch,iternum, device, out_path, logging=None):
    ...
    os.system("bash -c 'source ~/anaconda3/etc/profile.d/conda.sh &&  conda activate modulardevelop && python function/modulardevelop/demo/run_search.py --data "+dataset+" --block_type "+model+" --tuner "+tuner+" --init "+init+ " --epoch "+str(epoch)+" --iternum "+str(iternum)+" --save_dir "+out_path+"'")
    logging.info("模型搜索完成......")
    ...
    os.system("bash -c 'source ~/anaconda3/etc/profile.d/conda.sh &&  conda activate modulardevelop && python function/modulardevelop/demo/run_convert.py --data "+dataset+" --model_path "+out_path+"/best_model.h5 --target_bk torch  --save_dir "+out_path+"/target_torch'")
