...
class CAM(object):
    ...
    def _get_weight(self):
        ...
        self.weight = np.squeeze(params[-2].data.cpu().numpy())
...
class CAM_tensor(object):
    ...
    def __call__(self, inputs, index):
        ...
        cam = torch.nn.functional.interpolate(cam.unsqueeze(0).unsqueeze(0), (32, 32), mode='bilinear')
        cam = cam.squeeze()
        # resize to 224*224
        # cam = cv2.resize(cam, (32, 32))
...
class GradCAM_tensor(object):
    ...
    def __call__(self, inputs, index):
        ...
        weight = gradient.mean(axis=(1,2))  # [C]
...
class GradCamPlusPlus_tensor(GradCAM_tensor):
    def __init__(self, net, layer_name):
        super(GradCamPlusPlus_tensor, self).__init__(net, layer_name)
