...
class FlowLine(object):
    def __init__(self, model, dataset='mnist', eps=4, attack_methods=['fgsm'], device='cuda', batch_size=128, log_func=None, ori_loader=None, adv_loader=None):
        self.model = model
    ...
    def run(self):
        ...
        if self.adv_loader:
            ...
            for method in self.adv_loader:
                self.write_logs(f"启动{method}方法攻击测试")
                results['attack_results'][method] = self.calc_ori_acc(self.adv_loader[method])
                acc = results['attack_results'][method]
                self.write_logs(f'执行{method}算法，准确率为：{acc}')
        else:
