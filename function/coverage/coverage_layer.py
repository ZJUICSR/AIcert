...
class NCoverage_with_output_of_conv_kernel():
    ...
    def update_coverage_step(self, inputs):
        ...
        for idx, (layer_name, layer_output) in enumerate(self.activation.items()):
            ...
            for data in scaled:
                top_k_list = torch.argsort(data,descending=True)[0:int(self.k*len(self.cov_dict[layer_name]))]
                # print('-'*100)
                # print(len(top_k_list))
                # print(len(self.cov_dict[layer_name]))
                # print(top_k_list)
                for top_k_neuron in top_k_list:
                    self.cov_dict[layer_name][top_k_neuron] = True
...
if __name__ == '__main__':
    ...
    # # # model = torchvision.models.vgg13(False)
    # model = LeNet5()
