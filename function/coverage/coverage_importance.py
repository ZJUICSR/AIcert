...
def get_imp_neus(net, n_imp, layer_names, trainloader: DataLoader, result_file, pic_savepath, visualization=True, logging=None):
    ...
    json_data['reldraw'] = {}
    json_data['reldraw']['0'] = reldraw[0]
    json_data['reldraw']['1'] = reldraw[1]
    json_data['reldraw']['2'] = reldraw[2]
    json_data['reldraw']['3'] = reldraw[3]
    json_data['reldraw']['4'] = reldraw[4]
    ...
    idx = idx[-n_imp:]
    logging.info('Top ' + str(n_imp) + ' Important Neurons: ' + str(idx))
    # print('Top ' + str(n_imp) + ' Important Neurons: ' + str(idx))
    return rels, idx, json_data
...
def get_dataloader_cifar(train, batch_size=2, input_size=(32, 32)):
    transform_fn = Compose([Resize(input_size), ToTensor(), ])
    dataset = CIFAR10('./dataset/data', download=True, train=train, transform=transform_fn)
    dataloader = torch.utils.data.DataLoader(dataset, batch_size=batch_size, shuffle=True)
    return dataloader
...
def run_importance_coverage(net: Module, trainloader: DataLoader, testloader: DataLoader, n_imp: int, clus: int, pic_savepath, filename, log_func=None):
    ...
    rels, imp_neus, json = get_imp_neus(net, n_imp, layer_names, trainloader, filename, pic_savepath, logging=log_func)
    ...
    json_data = {}
    json_data['ImportanceNeuronsCoverage'] = {}
    json_data['ImportanceNeuronsCoverage']['Coverage'] = Coverage[-1]
    json_data['ImportanceNeuronsCoverage']["reldraw"] = json
    return json_data
...
def run_visualize_importance(dataset, model_type, model_path, n_imp, clus, out_path, logging=None):
    ...
    # print(model)
    return run_importance_coverage(model, train_loader, test_loader, n_imp, clus, out_path, "", log_func=logging)
