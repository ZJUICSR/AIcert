...
def run_visualize(model, dataloader, result_file, model_type='lenet', file_names=['1', '2', '3', '4'], outputdir='',
                  number_of_image=10, log_func=None):
    if model_type == 'lenet':
    ...
    for i, (x, y) in enumerate(dataloader):
        ...
        with open(result_file, 'w') as file_obj:
            ...
            json_data['coverage_test_yz']['coverage_visual_image'] = {'result_dir': outputdir,
                                                                      'image': ['%s.svg' % (n) for n in range(i)]}
...
if __name__ == '__main__':
    ...
    dataloader = get_dataloader_cifar(False, batch_size=16, input_size=input_size)

    # model_type参数有lenet和vgg两种取值
    run_visualize(model, dataloader=dataloader, result_file='keti2(2)(1).json', model_type='vgg16',
