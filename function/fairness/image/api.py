...
def model_evaluate(dataset_name, model_path = '', model_name="Resnet50", metrics=['mPre', 'mFPR', 'mFNR', 'mTNR', 'mTPR', 'mAcc', 'mF1', 'mBA'], test_mode=True):
    
    model, opt = collect_args(model_path, dataset_name)
    if model_path == '':
       model.model_path = os.path.join('record/' + opt['experiment'] + '/', opt['experiment_name'])
