...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        x, activation1, activation2, activation3, activation4 = self.get_final_fm(x)
        x = self.fc(x)
    ...
    def get_final_fm(self, x: Tensor) -> Tensor:
        x, activation1, activation2, activation3, activation4 = self.get_fm(x)
        x = self.avg_pool(x)
        x = x.view(x.size(0), -1)
