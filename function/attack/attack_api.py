...
class EvasionAttacker():
    ...
    def generate(self, method: str="FastGradientMethod", save_num: int = 0, **kwargs) -> None:
        ...
        self.save_examples(save_num, real_lables, cln_examples, clean_predictions, adv_examples, adv_predictions)
        return adv_examples
...
class BackdoorAttacker():
    ...
    # def data_poison(self, xs: np.ndarray, ys: np.ndarray, target: np.ndarray, source: np.ndarray, pp_poison: float, batch_size=256):
    #     # 进行复制避免原数据被更改
    #     x, y = xs.copy(), ys.copy()
    #     total_num = len(x)

    #     if self.method == "PoisoningAttackBackdoor":
    #         self.attack = PoisoningAttackBackdoor(self.add_backdoor)
    ...
    #         x_p, y_p = self.attack.poison(x_needp, target)
    ...
    #         self.attackerbase = PoisoningAttackBackdoor(self.add_backdoor)
    #         self.attack = PoisoningAttackCleanLabelBackdoor(target=target, pp_poison=pp_poison, proxy_classifier=self.classifier, batch_size=batch_size, backdoor=self.attackerbase)
    #         return self.attack.poison(x, y)
    ...
    #         target_indices = all_indices[np.all(ys == target, axis=1)]
    #         print(int(len(target_indices)*pp_poison))
    #         num_poison = int(pp_poison * len(target_indices))
    #         target_selected_indices = np.random.choice(target_indices, num_poison)
    #         xp = x.take(target_selected_indices, axis=0)

    #         all_indices = np.arange(len(x))
    #         source_indices = all_indices[np.all(ys == source, axis=1)]
    #         source_selected_indices = np.random.choice(source_indices, num_poison)
    #         backdoor_item = x.take(source_selected_indices, axis=0)

    #         for index in tqdm(range(len(xp)), desc="Feature collision"):
    #             self.attack = FeatureCollisionAttack(classifier=self.classifier, target=np.expand_dims(backdoor_item[index], axis=0), feature_layer=fl)
    #             xp[index] = self.attack.poison(np.expand_dims(xp[index], axis=0))[0]
            
    #         for i, j in enumerate(target_selected_indices):
    #             x[j,:,:,:] = xp[i,:,:,:]

    #         return x, y, target_selected_indices
        
    #     elif self.method == "PoisoningAttackAdversarialEmbedding":
    #         # PoisoningAttackAdversarialEmbedding
    #         self.attackerbase = PoisoningAttackBackdoor(self.add_backdoor)
    #         self.attack = PoisoningAttackAdversarialEmbedding(backdoor=self.attackerbase, device=self.device)
    #         select_list = random.sample(range(0, total_num), int(total_num*pp_poison))
    #         print(int(total_num*pp_poison))
    #         x_needp = x.take(select_list, axis=0)
    #         x_p, y_p = self.attack.poison(x_needp, target)
    ...
    # def finetune(self, x_train: np.ndarray, y_train: np.ndarray, x_test: np.ndarray, y_test: np.ndarray, select_list: list=[], test_list: list=[], num_epochs: int=40, batch_size: int=700, lr=0.001, alpha=1):
    #     """
    #     模型在投毒样本上重新训练和微调
    #     """
    #     # if self.method == "PoisoningAttackAdversarialEmbedding":
    #     #     self.attack.fintune(self.model, x_train, y_train, select_list, x_test, y_test, test_list, num_epochs=num_epochs, batch_size=batch_size, lr=0.001, alpha=alpha)
    ...
    #             all, clean, backdoor = self.evaluate(x_test, y_test, test_list)
    #         else:
    #             tmp_model,_,_ = self.attack.fintune(self.model, i, x_train, y_train, select_list, batch_size, lr, alpha, tmp_model)
    #             all, clean, backdoor = self.evaluate(x_test, y_test, test_list)
    #         print((all, clean, backdoor))
