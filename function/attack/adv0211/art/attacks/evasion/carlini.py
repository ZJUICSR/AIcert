...
class CarliniWagner(EvasionAttack):
    attack_params = EvasionAttack.attack_params + ["norm"] + CarliniL2Method.attack_params + CarliniL0Method.attack_params + CarliniLInfMethod.attack_params
    attack_params = list(set(attack_params))
    def __init__(self, estimator: "CLASSIFIER_CLASS_LOSS_GRADIENTS_TYPE") -> None:
        self.norm = 0
        self.myestimator = estimator
        
        if self.norm not in [0, 2, np.inf, "inf"]:
            raise ValueError('Norm order must be either 0, 2, `np.inf` or "inf".')
    ...
    def generate(self, x: np.ndarray, y: Optional[np.ndarray] = None) -> np.ndarray:
        ...
        if self.norm == 2:
            self.CWAttack = CarliniL2Method(self.myestimator)
            method = "CarliniL2Method"
        elif self.norm == 0:
            self.CWAttack = CarliniL0Method(self.myestimator)
            method = "CarliniL0Method"
        else:
            self.CWAttack = CarliniLInfMethod(self.myestimator)
            method = "CarliniLInfMethod"
        
        for i in self.attack_params:
            if hasattr(self, i) and hasattr(self.CWAttack, i):
                setattr(self.CWAttack, i, getattr(self, i))
            elif hasattr(self, i) and not hasattr(self.CWAttack, i) and i != "norm":
                raise ValueError("{} is not the parameter of {}".format(i, method))
            else:
                pass
        
        return self.CWAttack.generate(x, y)