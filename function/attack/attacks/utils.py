...
if TYPE_CHECKING:
    ...
    from function.attack.estimators.classification.blackbox import BlackBoxClassifier
    from function.attack.estimators.classification.catboost import CatBoostARTClassifier
    ...
    from function.attack.estimators.classification.query_efficient_bb import QueryEfficientGradientEstimationClassifier
    from function.attack.estimators.classification.scikitlearn import (
    ...
    from function.attack.estimators.classification.tensorflow import TensorFlowClassifier, TensorFlowV2Classifier
    from function.attack.estimators.classification.xgboost import XGBoostClassifier
    # from function.attack.estimators.certification.derandomized_smoothing.derandomized_smoothing import BlockAblator, ColumnAblator
    # from function.attack.estimators.generation import TensorFlowGenerator
    # from function.attack.estimators.generation.tensorflow import TensorFlowV2Generator
    from function.attack.estimators.object_detection.object_detector import ObjectDetector
    ...
    # from function.attack.estimators.regression.scikitlearn import ScikitlearnRegressor
    # from function.attack.estimators.speech_recognition.pytorch_deep_speech import PyTorchDeepSpeech
    # from function.attack.estimators.speech_recognition.tensorflow_lingvo import TensorFlowLingvoASR
    from function.attack.estimators.tensorflow import TensorFlowV2Estimator
...
# 计算攻击成功率，在所有样本上进行(包括原本分类错误的样本的对抗样本)
def compute_success(
    classifier: "CLASSIFIER_TYPE",
    x_clean: np.ndarray,
    labels: np.ndarray,
...
def compute_attack_success(
    ...
    for i in range(len(real_labels)):
        # 仅仅对正确分类的样本考虑攻击成功率
        if np.argmax(real_labels[i]) == np.argmax(clean_preds[i]):
            cover_num += 1
            if targeted:
                if np.argmax(adv_preds[i]) == np.argmax(target_preds[i]):
                    attack_sucess_num += 1
            else:
                if np.argmax(adv_preds[i]) != np.argmax(clean_preds[i]):
                    attack_sucess_num += 1
    return cover_num / len(real_labels), attack_sucess_num / cover_num

# 攻击代码中用来计算准确率的函数，外部接口应当避免使用
...
# 计算预测准确率
def compute_predict_accuracy(preds: np.ndarray, labels: np.ndarray) -> Tuple[float, float]:
    num_correct = np.sum(np.argmax(preds, axis=1) == np.argmax(labels, axis=1))
...
def load_cifar10 (
    ...
    if normalize:
        ...
        max_value = np.amax(x_test) if np.amax(x_test) > max_value else max_value
...
def load_stl() -> DATASET_TYPE:
    """
    Loads the STL-10 dataset from `config.MY_DATA_PATH` or downloads it if necessary.
...
def load_iris(raw: bool = False, test_set: float = 0.3) -> DATASET_TYPE:
    """
    Loads the UCI Iris dataset from `config.MY_DATA_PATH` or downloads it if necessary.
...
def load_nursery(
    ...
    """
    Loads the UCI Nursery dataset from `config.MY_DATA_PATH` or downloads it if necessary.
