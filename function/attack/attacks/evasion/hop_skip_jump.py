...
from function.attack.estimators.classification import ClassifierMixin
from function.attack.attacks.utils import compute_success, to_categorical, check_and_transform_label_format, get_labels_np_array
...
class HopSkipJump(EvasionAttack):
    ...
    def generate(self, x: np.ndarray, y: Optional[np.ndarray] = None, **kwargs) -> np.ndarray:
        ...
        if mask is not None:
            if len(mask.shape) == len(x.shape):
                mask = mask.astype(MY_NUMPY_DTYPE)
            else:
                mask = np.array([mask.astype(MY_NUMPY_DTYPE)] * x.shape[0])
        else:
    ...
    def _init_sample(
        ...
        if self.targeted:
            ...
            if adv_init is not None and init_pred != y_p:
                return adv_init.astype(MY_NUMPY_DTYPE), y_p
    ...
    def _compute_update(
        ...
        if self.norm == 2:
            rnd_noise = np.random.randn(*rnd_noise_shape).astype(MY_NUMPY_DTYPE)
        else:
            rnd_noise = np.random.uniform(low=-1, high=1, size=rnd_noise_shape).astype(MY_NUMPY_DTYPE)
        ...
        f_val = f_val.astype(MY_NUMPY_DTYPE)
