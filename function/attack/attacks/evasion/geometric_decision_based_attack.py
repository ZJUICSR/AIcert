...
from function.attack.attacks.utils import check_and_transform_label_format, get_labels_np_array
if TYPE_CHECKING:
...
class GeoDA(EvasionAttack):
    ...
    def generate(self, x: np.ndarray, y: Optional[np.ndarray] = None, **kwargs) -> np.ndarray:
        ...
        if os.path.exists(path):
            self.sub_basis = np.load(path).astype(MY_NUMPY_DTYPE)
        else:
            self.sub_basis = self._generate_2d_dct_basis(sub_dim=self.sub_dim, res=image_size).astype(MY_NUMPY_DTYPE)
            # np.save(path, self.sub_basis)

        for i in trange(x.shape[0], desc="GeoDA - samples", position=0):
            x_i = x[[i]]
            y_i = y[[i]]
            ...
            for k in trange(self.iterate, desc="GeoDA - steps", position=1):
    ...
    def _find_random_adversarial(self, x: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        while not self._is_adversarial(x_perturbed, y):
            nb_calls += 1
            perturbation = np.random.normal(size=x.shape).astype(MY_NUMPY_DTYPE)
    ...
    def _sub_noise(self, num_noises: int, basis: np.ndarray):
        ...
        sub_noise = np.array(np.matmul(basis, noise).transpose((1, 0)).astype(MY_NUMPY_DTYPE))
