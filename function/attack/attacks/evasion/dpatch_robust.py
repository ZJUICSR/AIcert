...
if TYPE_CHECKING:
    from function.attack.attacks.utils import OBJECT_DETECTOR_TYPE
...
class RobustDPatch(EvasionAttack):
    ...
    def __init__(
        ...
        if self.estimator.clip_values is None:
            self._patch = np.zeros(shape=patch_shape, dtype=config.MY_NUMPY_DTYPE)
        else:
            self._patch = (
