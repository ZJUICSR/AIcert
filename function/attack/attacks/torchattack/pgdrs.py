...
class PGDRS(EvasionAttack):
    ...
    def __init__(self, classifier, eps=8/255, alpha=2/255, steps=10, noise_type="guassian", noise_sd=0.5, noise_batch_size=5, batch_max=2048):
    ...
    def generate(self, x : np.ndarray, y : Optional[np.ndarray] = None):
        ...
        if x.shape[0]*self.noise_batch_size > self.batch_max:
            ...
            y_split = torch.split(y, split_size_or_sections=split_num)
            x_list = []
