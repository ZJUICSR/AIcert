...
# 工具函数
from estimators.classification import PyTorchClassifier
...
class EvasionAttacker():
    def __init__(self, modelnet=ResNet18(1), modelpath: str="ckpt-resnet18-mnist_epoch3_acc0.9898.pth", 
...
class BackdoorAttacker():
    def __init__(
    self, modelnet=ResNet18(1), modelpath: str="./models/model_ckpt/ckpt-resnet18-mnist_epoch3_acc0.9898.pth", 
    dataset="mnist", datasetpath="./datasets/", nb_classes=10, datanormalize: bool = False, 
    ...
    def finetune(self, x_train: np.ndarray, y_train: np.ndarray, x_test: np.ndarray, y_test: np.ndarray, select_list: list=[], test_list: list=[], num_epochs: int=40, batch_size: int=700, lr=0.001, alpha=50):
        """
        模型在投毒样本上重新训练和微调
        """
    ...
    def backdoorattack(self, method: str="PoisoningAttackBackdoor", pp_poison: float=0.5, target: int=3, batch_size:int=700, num_epochs=40, lr=0.01, alpha=50, test_sample_num:int=2048, save_num: int=32):
        ...
        # 主要是为了区分开测试集和训练集
        x_test, y_test, test_plist = self.poison(x_select_tmp, y_select_tmp, self.target, pp_poison=0.5, backdoor_item=x_select_tmp[random.randint(1,len(x_select_tmp))][np.newaxis, :])
        # 根据比例进行训练集投毒
        x_train, y_train, plist= self.poison(x_train_tmp, y_train_tmp, self.target, pp_poison=pp_poison, backdoor_item=x_select_tmp[random.randint(1,len(x_select_tmp))][np.newaxis, :])
