...
if TYPE_CHECKING:
    from attacks.utils import CLASSIFIER_NEURALNETWORK_TYPE

# logger = logging.getLogger(__name__)


class PixelThreshold(EvasionAttack):
    attack_params = EvasionAttack.attack_params + ["th", "es", "max_iter", "targeted", "batch_size"]
    ...
    def rescale_input(self, x):
        """Rescale inputs"""
        x = x.astype(MY_NUMPY_DTYPE) / 255.0
    ...
    def generate(self, x: np.ndarray, y: Optional[np.ndarray] = None, **kwargs) -> np.ndarray:
        ...
        for image, target_class in tqdm(tzip(x, y), desc="Pixel threshold"):
            if self.th is None:
                min_th = -1
                start, end = 1, 127
...
class PixelAttack(PixelThreshold):
    def __init__(
        ...
        print("Batch_size is not used in this attack.")
        super().__init__(classifier, th, es, max_iter, targeted)
        self.type_attack = 0
