...
# from estimators.classification.tensorflow import TensorFlowV2Classifier
...
from attacks.utils import get_labels_np_array, check_and_transform_label_format, compute_success_array, random_sphere
...
if TYPE_CHECKING:
    from attacks.utils import CLASSIFIER_LOSS_GRADIENTS_TYPE, OBJECT_DETECTOR_TYPE
    import torch
    from estimators.classification.pytorch import PyTorchClassifier
...
class ProjectedGradientDescentCommon():
    attack_params = EvasionAttack.attack_params + ["norm","eps","targeted","batch_size"] + ["max_iter", "random_eps", "verbose"]
...
class ProjectedGradientDescentPyTorch(ProjectedGradientDescentCommon):
    ...
    def generate(self, x: np.ndarray, y: Optional[np.ndarray] = None, **kwargs) -> np.ndarray:
        ...
        for (batch_id, batch_all) in enumerate(
            tqdm(data_loader, desc="PGD - Batches", leave=False, disable=False)
        ):
