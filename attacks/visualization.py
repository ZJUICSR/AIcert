...
def save_image(image_array: np.ndarray, f_name: str) -> None:
    """
    Saves image into a file inside `MY_DATA_PATH` with the name `f_name`.
    ...
    """
    file_name = os.path.join(config.MY_DATA_PATH, f_name)
...
def plot_3d(
    ...
    :param save:  When set to True, saves image into a file inside `MY_DATA_PATH` with the name `f_name`.
    ...
    if save:  # pragma: no cover
        file_name = os.path.realpath(os.path.join(config.MY_DATA_PATH, f_name))
        folder = os.path.split(file_name)[0]
